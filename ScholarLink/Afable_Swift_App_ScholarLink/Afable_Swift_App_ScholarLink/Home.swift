...
struct TutorDetailView: View { // TUTOR PROFILE FOR BOOKING
    ...
    @State private var tutorReviews: [Session] = []
    ...
    func fetchTutorRatings() async {
        do {
            ...
            let reviews = sessions.filter { session in
                ...
                if !tutorReviews.isEmpty {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Reviews")
                            .font(.headline)
                        VStack(spacing: 12) {
                            ForEach(tutorReviews, id: \.id) { reviewSession in
                                VStack(alignment: .leading, spacing: 6) {
                                    HStack(spacing: 6) {
                                        if let rating = reviewSession.rating {
                                            HStack(spacing: 2) {
                                                ForEach(1...5, id: \.self) { index in
                                        ...
                                        Spacer()
                                        Text(reviewSession.sessionDate, style: .date)
                                            .font(.caption2)
                                            .foregroundColor(.gray)
                                    }
                                    if let text = reviewSession.review {
                                        Text(text)
                                            .font(.subheadline)
                                    }
                                    Text("– \(reviewSession.studentName)")
                                        .font(.caption2)
                                        .foregroundColor(.gray)
                                }
                                .padding(10)
